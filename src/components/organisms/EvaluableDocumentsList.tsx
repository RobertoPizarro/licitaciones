...
const EvaluableDocumentsList: React.FC<EvaluableDocumentsListProps> = ({
    ...
    // Hardcoded documents as per implementation plan
    ...
    return (
        <div className={`evaluable-documents-list ${disabled ? 'disabled' : ''}`}>
            ...
            {!disabled && documentCategories.map((category, categoryIndex) => (
                <div key={categoryIndex} className="doc-category">
                    <div className="doc-category-header">
                        <div className="doc-category-icon">{category.icon}</div>
                        <h4 className="doc-category-title">{category.title}</h4>
                    </div>
